...
/**
 * @author Fleyer
 * <p> Board creation on 29.03.2023 at 13:02
 */
@FieldDefaults(level = AccessLevel.PRIVATE)
public class Board {

    static Map<Class<?>, Field[]> PACKETS = new HashMap<>(8);
    static String[] COLOR_CODES = Arrays.stream(ChatColor.values())
            .map(Object::toString)
            .toArray(String[]::new);
    static VersionType VERSION_TYPE;
    // Packets and components
    static Class<?> CHAT_COMPONENT_CLASS;
    static Class<?> CHAT_FORMAT_ENUM;
    static Object EMPTY_MESSAGE;
    static Object RESET_FORMATTING;
    static MethodHandle MESSAGE_FROM_STRING;
    static MethodHandle PLAYER_CONNECTION;
    static MethodHandle SEND_PACKET;
    static MethodHandle PLAYER_GET_HANDLE;
    // Scoreboard packets
    static BoardReflection.PacketConstructor PACKET_SB_OBJ;
    static BoardReflection.PacketConstructor PACKET_SB_DISPLAY_OBJ;
    static BoardReflection.PacketConstructor PACKET_SB_SCORE;
    static BoardReflection.PacketConstructor PACKET_SB_TEAM;
    static BoardReflection.PacketConstructor PACKET_SB_SERIALIZABLE_TEAM;
    // Scoreboard enums
    static Class<?> ENUM_SB_HEALTH_DISPLAY;
    static Class<?> ENUM_SB_ACTION;
    static Object ENUM_SB_HEALTH_DISPLAY_INTEGER;
    static Object ENUM_SB_ACTION_CHANGE;
    static Object ENUM_SB_ACTION_REMOVE;
    ...
    static {
        try {
            ...
            val craftPlayerClass = BoardReflection.obcClass("entity.CraftPlayer");
            val craftChatMessageClass = BoardReflection.obcClass("util.CraftChatMessage");
            val entityPlayerClass = BoardReflection.nmsClass("server.level", "EntityPlayer");
            val playerConnectionClass = BoardReflection.nmsClass("server.network", "PlayerConnection");
            val packetClass = BoardReflection.nmsClass("network.protocol", "Packet");
            val packetSbObjClass = BoardReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardObjective");
            val packetSbDisplayObjClass = BoardReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardDisplayObjective");
            val packetSbScoreClass = BoardReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardScore");
            val packetSbTeamClass = BoardReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardTeam");
            val sbTeamClass = VersionType.V1_17.isHigherOrEqual()
            ...
            val sendPacketMethod = Arrays.stream(playerConnectionClass.getMethods())
                    .filter(m -> m.getParameterCount() == 1 && m.getParameterTypes()[0] == packetClass)
                    .findFirst().orElseThrow(NoSuchMethodException::new);

            MESSAGE_FROM_STRING = lookup.unreflect(craftChatMessageClass.getMethod("fromString", String.class));
            CHAT_COMPONENT_CLASS = BoardReflection.nmsClass("network.chat",
                    "IChatBaseComponent");
            ...
            PLAYER_GET_HANDLE = lookup.findVirtual(craftPlayerClass,
                    "getHandle", MethodType.methodType(entityPlayerClass));
            ...
            PACKET_SB_SERIALIZABLE_TEAM = sbTeamClass == null ?
                    null
                    : BoardReflection.findPacketConstructor(sbTeamClass, lookup);
            ...
            if (VersionType.V1_8.isHigherOrEqual()) {
                val enumSbActionClass = VersionType.V1_13.isHigherOrEqual()
                        ? "ScoreboardServer$Action"
                        : "PacketPlayOutScoreboardScore$EnumScoreboardAction";
                ENUM_SB_HEALTH_DISPLAY = BoardReflection.nmsClass("world.scores.criteria",
                        "IScoreboardCriteria$EnumScoreboardHealthDisplay");
                ENUM_SB_ACTION = BoardReflection.nmsClass("server", enumSbActionClass);
                ENUM_SB_HEALTH_DISPLAY_INTEGER = BoardReflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY,
                        "INTEGER", 0);
    ...
    @Getter
    String title = ChatColor.RESET.toString();
    @Getter
    ...
    public synchronized void updateLine(int line, String text) {
        ...
        try {
            ...
            val newLines = new ArrayList<>(this.lines);
    ...
    public synchronized void updateLines(Collection<String> lines) {
        ...
        val oldLines = new ArrayList<>(this.lines);
        this.lines.clear();
        ...
        try {
            if (oldLines.size() != linesSize) {
                val oldLinesCopy = new ArrayList<>(oldLines);
    ...
    private void sendScorePacket(int score, ScoreboardAction action) throws Throwable {
        ...
        if (VersionType.V1_8.isHigherOrEqual()) {
            setField(packet, ENUM_SB_ACTION, action == ScoreboardAction.REMOVE
    ...
    private void sendTeamPacket(int score, TeamMode mode) throws Throwable {
        ...
        val maxLength = hasLinesMaxLength() ? 16 : 1024;
    ...
    private void setComponentField(Object packet, String value, int count) throws Throwable {
        ...
        var i = 0;
        for (val field : PACKETS.get(packet.getClass())) {
            if ((field.getType() == String.class || field.getType() == CHAT_COMPONENT_CLASS) && count == i++) {
                field.set(packet, value.isEmpty() ? EMPTY_MESSAGE :
